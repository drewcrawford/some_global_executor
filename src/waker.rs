...
impl WakeInternal {
    ...
    fn wake_by_ref(&self) {
        match self.inline_wake.compare_exchange(
            NOT_WOKEN,
            WOKEN,
    ...
    pub fn reset(&self) {
        self.inline_wake
            .store(NOT_WOKEN, std::sync::atomic::Ordering::Relaxed);
    }

    pub fn check_wake<F>(&self, alert: F)
    where
        F: FnOnce() + 'static + Send,
    {
    ...
    unsafe {
        ...
        (
            std::task::Waker::from_raw(std::task::RawWaker::new(
                Arc::into_raw(wake_internal.clone()) as *const (),
                &VTABLE,
            )),
            wake_internal,
        )
