...
async fn test_static_inline() {
    let (c, f) = continuation();
    let mut global_executor =
        some_global_executor::Executor::new("test_static_inline".to_string(), 100);
    let systems_task = Task::without_notifications(
        "Systems".to_string(),
        Configuration::default(),
        async move {
            //need a non-send task to bring them up
            let systems_inner_task = Task::without_notifications(
                "Systems (non-Send)".to_string(),
                Configuration::default(),
                async move {
                    c.send(());
                },
